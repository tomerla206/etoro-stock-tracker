...
async def scan_one(context, sem, ticker, results, log_state):
    async with sem:
        page = None
        try:
            ...
            if target is None and div_pct is None:
            ...
            # Covers page creation failing too (e.g. the browser driver
            # connection died) - must still count this ticker as done so the
            # whole batch doesn't silently stall/undercount, and must signal
            # the browser is unhealthy so main() can relaunch it.
            results["errors"].append(ticker)
            if page is None:
                log_state["browser_dead"] = True
        finally:
            if page is not None:
                try:
                    await page.close()
...
async def main():
    ...
    log_state = {"done": 0, "total": len(tickers), "start": time.time(), "browser_dead": False}
    ...
    with open(ROOT / "MEGASCAN_YAHOO_RESULTS.md", "w", encoding="utf-8") as f:
        ...
        if log_state["browser_dead"]:
            f.write("**The browser driver connection died partway through this run** "
                    "(same failure mode seen twice now - likely a host machine issue, "
                    "not a script bug). Progress is checkpointed - just re-run "
                    "`python megascan_yahoo.py` and it will pick up only the "
                    f"{len(still_missing)} tickers still missing, not start over.\n")
