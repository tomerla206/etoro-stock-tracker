...
5. Click the **"Orders"** tab too (same page, same button-by-text pattern) — shows pending orders not yet filled (status "Received" etc.), with columns Asset/Current/Invested/Execute At/SL/TP/Status/Leverage. If empty, it says "There are no pending orders" — this itself is useful information (report it). **These must also be written to `portfolio_*.tsv` as `PENDING` rows (see step 8) — not just reported in chat.** The site already fully supports a distinct orange `pf-pending` badge for this status (`merge_portfolio.py`'s `repl_row_for_badge()`, CSS in `part1_fixed.html`'s `.pf-badge.pf-pending`) — it was simply never being fed any data, since this step historically only reported pending orders verbally and never persisted them.
...
7. Compare the freshly parsed data against the stored `portfolio_virtual.tsv` / `portfolio_real.tsv` (format: `TICKER\tName\tUnits\tAvgOpen\tPL\tPLPercent\tNetValue\tStatus`) and `portfolio_tp.tsv` / `portfolio_real_tp.tsv` (format: `TICKER\tTP` or just `TICKER` with no second field if no TP set) to find:
   ...
   - Report the **pending orders** list from step 5 too, AND write them into the stored files per step 8 below (they used to be reported only, never persisted — that's why the orange PENDING badge stopped appearing on the site even though the code for it still exists).
8. Write the fresh data back to all 4 files (preserve company names from the old file via a ticker→name lookup dict, since the live page doesn't show a clean company name string easily). For each pending order from step 5, append/update a row in `portfolio_virtual.tsv`/`portfolio_real.tsv` with `Status=PENDING`, mapping the Orders-tab columns onto the 8-field format as: `Units=Invested/ExecuteAt` (approx units the order would buy), `AvgOpen=Execute At` (the target fill price), `PL=0`, `PLPercent=0`, `NetValue=Invested`. A pending order has no TP-badge-relevant TP file entry unless the order itself set one (rare — usually leave `portfolio_tp.tsv`/`portfolio_real_tp.tsv` untouched for it). When a pending order later fills (ticker appears in the Manual Trades list next scan), overwrite its row to `Status=HELD` with real Units/AvgOpen/PL as normal — same as any other new position. Run `python merge_portfolio.py`, then reassemble (`cat part1_fixed.html all_rows.html part3.html > nasdaq-stocks.html`), then run `python build_red_pl_report.py` to refresh `red_pl_report.html` (the Virtual red-P/L report, served at `http://localhost:8791/red_pl_report.html`) against the just-updated `portfolio_virtual.tsv` — cheap, local-only, no live session needed, so just always re-run it here.
...
## Code name `SCAN EXIT PLAN` (added 2026-09-19) — per-position exit targets for Virtual

**Trigger phrase**: "SCAN EXIT PLAN", "תוכנית יציאה", "מתי למשוך". Runs `python build_exit_plan_report.py`, which re-renders `exit_plan_report.html`: every `HELD` Virtual position (regardless of current P/L sign, unlike SCAN LOSSES which only covers losers), with entry/current price, a suggested exit goal, the Upside% from the current price to that goal, the total profit% from entry if the goal is hit, and (for TP-based goals only) an estimated time to get there.

**Goal price logic**: uses the eToro Take Profit (`portfolio_tp.tsv`) if the user already set one for that ticker (badge "TP"), otherwise falls back to the analyst average target (badge "קונצנזוס"), with the same broken-target heuristic as SCAN LOSSES. **Deliberately does not compute a time estimate for consensus-target goals** - since the analyst target IS the ~12-month growth assumption by construction, projecting "time to reach the target" using a growth rate derived from that same target would always trivially return ~12 months regardless of the actual numbers, which is not real information. Those rows show a fixed "יעד קונצנזוס — טווח של כ-12 חודש" label instead. Only TP-based goals (a price target independent of the analyst-implied growth curve) get a real computed month estimate, using the analyst-implied monthly growth rate to project how long reaching that specific TP would take.

**Not a live scan**, same as SCAN LOSSES - pure re-render of `portfolio_virtual.tsv` + `portfolio_tp.tsv` + `all_rows.html`, safe to re-run standalone anytime those are refreshed. Not yet wired into the SCAN HOLDINGS/fundamentals_scan auto-chains (unlike SCAN LOSSES) - run it manually after a portfolio change until the user asks for it to be automated too. Not gitignored, served at `http://localhost:8791/exit_plan_report.html`, linked from the main site's header next to the Red P/L button.

Also shows three money columns per row (added 2026-09-19 at the user's request): הושקע (`invested` = units×avgopen), P/L בדולרים (`pl_dollar`, straight from `portfolio_virtual.tsv`'s own pl field, not re-derived from plpct - avoids compounding rounding error), and שווי נוכחי (`netvalue`, should always equal invested+pl - `validate_portfolio_data.py` checks this). Table also has a `#` row-number column that renumbers live after any sort (see `applySort`'s renumbering loop - clicking a header re-sorts the DOM rows then rewrites `td.rownum`'s text/dataset in the new order, since the numbers must reflect display position, not original data order).

## Code name `SCAN CHECK` (added 2026-09-19) — data-integrity check for the two private reports

**Trigger phrase**: "SCAN CHECK", "תבדוק באגים", "תבדוק שגיאות בנתונים". Runs `python validate_portfolio_data.py` - a read-mostly sanity checker (only touches a file when it finds and fixes a `netvalue` arithmetic mismatch) covering: `portfolio_virtual.tsv`/`portfolio_real.tsv` (field count, numeric parse, duplicate tickers, `netvalue == units*avgopen+pl` - auto-corrects `netvalue` in place if it drifts, which is exactly the class of bug that silently corrupts every downstream money column), `portfolio_tp.tsv`/`portfolio_real_tp.tsv` (duplicate tickers, stale TP entries for tickers no longer HELD), every held ticker's presence as a row in `all_rows.html` (a miss here means broken name/target lookups downstream), and `exit_plan_report.html`/`red_pl_report.html`'s own embedded DATA JSON (parses cleanly, and each row's `invested + pl_dollar == netvalue`, which catches a template/render bug independently of the source-data check).

**Found real bugs on its first run**: `portfolio_real.tsv`'s SAA.L and RHM.DE rows had stale `netvalue` fields that didn't match `units*avgopen+pl` (pre-existing, from before this script existed) - auto-corrected, then `red_pl_report.html`/`exit_plan_report.html` were rebuilt to pick up the fix. **Not wired into any auto-chain yet** - run it manually after portfolio-data changes, or whenever asked to check for bugs in this pipeline; exits 1 if it finds anything not auto-fixable (still exits 0 after auto-fixing, since fixed issues aren't failures), so it's safe to use as a CI-style gate later if the user wants that.
