...
# Path to actual installed Chrome user profile
REAL_CHROME_PROFILE = os.path.expanduser(r"~\AppData\Local\Google\Chrome\User Data")
    ...
    if os.path.exists(filepath):
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                parts = line.split("\t")
                if parts:
                    # Ignore NOFAQ without price targets if we want to retry logged in
...
def process_ticker(page, ticker, company_name, current_price):
    ...
    print_log(f" Navigating tab live to: {url}")
    try:
    ...
    if "It looks like something went wrong" in page_text or "Error 1015" in page_text:
        return "BLOCKED", None
    ...
    low, avg, high = "", "", ""
    try:
        # Extract TipRanks prices if present on page
...
def run_agent_for_letter(page, letter, master_data):
    ...
    print_log(f"\n==========================================")
    print_log(f" Starting Live Agent for Letter {letter}")
    print_log(f" Remaining tickers: {len(worklist)}")
    print_log(f"==========================================\n")

    if not worklist:
        print_log(f"[!] Letter {letter} is already completed!")
        return True
...
def main():
    ...
    print_log("Launching Chrome using real user profile...")
    ...
    with sync_playwright() as p:
        ...
        print_log("\n" + "="*60)
        print_log(" Chrome is open with your real logged-in eToro account!")
        print_log(" Starting live navigation through remaining tickers...")
        print_log("="*60 + "\n")
        
        time.sleep(3)

        # Run Letter R agent (Forward)
