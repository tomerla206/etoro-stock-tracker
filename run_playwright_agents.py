...
def process_ticker(page, ticker, company_name, current_price):
    ...
    print_log(f" -> Live navigation to: {url}")
    try:
    ...
    # Signature check only for actual lockout/error, not logged-in pages
    if "Error 1015" in page_text or ("something went wrong" in page_text.lower() and "sign in" in page_text.lower()):
        return "BLOCKED", None
...
def run_agent_for_letter(page, letter, master_data):
    ...
    for i, ticker in enumerate(worklist, 1):
        ...
        print_log(f"[{letter}] ({i}/{len(worklist)}) Moving tab live to {ticker} ({info['name']})...")
...
def main():
    ...
    print_log("Launching Playwright Chromium browser...")
    ...
    with sync_playwright() as p:
        ...
        print_log("\n" + "="*60)
        print_log(" Please log in to eToro in the opened browser window.")
        print_log(" Waiting up to 60s for login detection...")
        print_log("="*60 + "\n")
        
        for sec in range(30):
            try:
                txt = page.inner_text("body")
                if "Logout" in txt or "Real" in txt or "Portfolio" in txt or "Watchlist" in txt:
                    print_log("[+] Logged in session detected!")
                    break
            except Exception:
                pass
            time.sleep(2)
